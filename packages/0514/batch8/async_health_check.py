...
FAILURE_COUNT_FILE = "async_failure_counts.json"
ALERT_THRESHOLD = 3
...
async def send_dingtalk_alert(session: aiohttp.ClientSession, webhook: str, url: str, 
                               errors: List[str], consecutive: int) -> bool:
    message = {
        "msgtype": "markdown",
        "markdown": {
            "title": "服务告警",
    ...
    try:
        async with session.post(webhook, json=message, timeout=10) as response:
            return response.status == 200
    except (aiohttp.ClientError, asyncio.TimeoutError):
        return False


async def send_wechat_alert(session: aiohttp.ClientSession, webhook: str, url: str, 
                             errors: List[str], consecutive: int) -> bool:
    message = {
        "msgtype": "markdown",
        "markdown": {
...
async def check_url(session: aiohttp.ClientSession, semaphore: asyncio.Semaphore,
                     url: str, keywords: List[str] = None, timeout: int = 10) -> Dict[str, Any]:
    """
    使用 Semaphore 控制并发数，避免连接池耗尽
    """
    result = {
        "url": url,
        "success": False,
        ...
        async with semaphore:
            async with session.get(url, timeout=timeout) as response:
                ...
                if response.status >= 400:
                    result["errors"].append(f"HTTP status code: {response.status}")
                else:
                    result["success"] = True
...
async def main():
    parser = argparse.ArgumentParser(description="Async HTTP service health check with alerts")
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        tasks = [
            check_url(session, semaphore, url, args.keywords, args.timeout)
        ...
        for result in results:
            ...
            if not result["success"] and should_send_alert(url_stats):
                ...
                if args.dingtalk_webhook:
                    if await send_dingtalk_alert(session, args.dingtalk_webhook, url, 
                                                  result["errors"], url_stats["consecutive_failures"]):
                        alert_sent = True
                
                if args.wechat_webhook:
                    if await send_wechat_alert(session, args.wechat_webhook, url, 
                                                result["errors"], url_stats["consecutive_failures"]):
                        alert_sent = True
